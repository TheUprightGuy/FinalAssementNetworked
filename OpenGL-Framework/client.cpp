...
//Uncomment this for the strangest bug you will even encounter
//#include "game.h"
...
void CClient::ProcessData(char* _pcDataReceived)
{
	...
	case HANDSHAKE:
	{
		...
		std::cout << _packetRecvd.MessageContent << " has joined." << std::endl;
		...
		oss << " ";
		oss << 90.0f;
		m_pConnectedPlayers.insert(std::pair<std::string, std::string>(_packetRecvd.MessageContent, oss.str()));
	...
	{
		TPacket _packet;
		_packet.Serialize(KEEPALIVE, "Yep"); //Hardcoded username; change to name as taken in via user input.
		SendData(_packet.PacketData);
		break;
	}
	case WORLD_UPDATE:
